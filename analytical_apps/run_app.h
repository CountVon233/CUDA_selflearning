...
void Run() {
  ...
  if (name.find("sssp") != std::string::npos) {
    ...
    } else if (name == "ppr_auto"){
      CreateAndQuery<OID_T, VID_T, VDATA_T, double, LoadStrategy::kBothOutIn,
                     PPRAuto, double, int, OID_T>(comm_spec, out_prefix, fnum, spec,
                                  FLAGS_ppr_d, FLAGS_ppr_mr,FLAGS_ppr_source);
                  
    }else if (name == "ppr"){
      CreateAndQuery<OID_T, VID_T, VDATA_T, double, LoadStrategy::kOnlyOut,
                     PPR, double, int, OID_T>(comm_spec, out_prefix, fnum, spec,
                                  FLAGS_ppr_d, FLAGS_ppr_mr,FLAGS_ppr_source);
    }
      else {
      LOG(FATAL) << "No avaiable application named : [" << name << "].";
